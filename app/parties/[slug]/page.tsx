...
export async function generateMetadata({ params }: PartyPageProps): Promise<Metadata> {
  ...
  return {
    ...
    description,
    alternates: {
      canonical: `https://jan-darshan.vercel.app/parties/${slug}`,
    },
    openGraph: {
      title,
      description,
      url: `https://jan-darshan.vercel.app/parties/${slug}`,
      siteName: "JanDarshan",
      type: "website",
      images: [
        {
...
export default async function PartyPage({ params }: PartyPageProps) {
  ...
  const canonicalUrl = `https://jan-darshan.vercel.app/parties/${slug}`;
